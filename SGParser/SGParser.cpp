...
// Может являться как собственным типом, так и классом так и шаблоном
// Так же может быть переопределенн от другого класса или типа
class type_t
{
public:
...
public:
	...
	void process_parse()
	{
		...
		for (const auto& symbol : code)
		{
			...
			// Произошло изменения буквы, теперь это что-то новое, но мы обработаем старое
